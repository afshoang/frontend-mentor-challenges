...
challenges.forEach(challenge => {
    ...
    const skillEles = challenge.skills.reduce((acc, curr) => {
        return acc + `
            <li data-type-skill=${curr}>${curr}</li>
        `
    }, '')

    cardEle.innerHTML = `
        <a href="./${challenge.folder}" class="card-img">
            <img src="./${challenge.folder}/design/demo.png" alt="Demo image">
        </a>

        <div class="card-content">
            <h3><a href="./${challenge.folder}">${challenge.name}</a></h3>

            <div class="card-skills">
                <div>
                    <ul>
                        <li class="link">
                            <a target="_blank" href="./${challenge.folder}">
